...
class _AuthorScreenState extends ConsumerState<AuthorScreen> {
    ...
    return Scaffold(
      appBar: widget.needAppBar
          ? const PreferredSize(
              ...
              child: AppBarWidget(automaticallyImplyLeading: true),
            )
          : null,
      body: booksAsync.when(
        loading: () => const Center(child: Loading()),
        error: (err, stack) =>
            Center(child: Text("${AppLocalizations.tr('error', lang)}: $err")),
        data: (books) {
          final authorBooks =
              books.where((book) => book.authorId == widget.author.id).toList();
          final bookCount = authorBooks.length;
                ...
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      Text(
                        AppLocalizations.tr('author', lang),
                        style: TextStyle(
                          fontSize: 16,
                      ...
                      Row(
                        ...
                        children: [
                          _buildAuthorStats(context, bookCount, lang),
                  ...
                  if (widget.author.bio != null)
                    _buildAboutSection(
                      AppLocalizations.tr('about_the_author', lang),
                      widget.author.bio!,
                      theme.colorScheme,
                    ),

                  Text(
                    AppLocalizations.tr('popular_works', lang),
                    style: TextStyle(
                      fontSize: 20,
                      fontWeight: FontWeight.bold,
                      color: theme.colorScheme.primary,
  ...
  Widget _buildAuthorStats(
      BuildContext context, int bookCount, AppLanguage lang) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceEvenly,
      children: [
        _buildStatItem(
            AppLocalizations.tr('books', lang), bookCount.toString()),
      ],
    );
  }
